...
public class AdminPortfolioService {
    ...
    public AdminDashboardResponse dashboard() { return new AdminDashboardResponse(projectRepository.count(),skillRepository.count(),experienceRepository.count(),educationRepository.count(),certificationRepository.count(),messageRepository.countByReadFalse()); }
    @Transactional(readOnly = true)
    public Profile getProfile(){return profileRepository.findAll().stream().findFirst().orElseThrow(()->notFound("Profile not configured"));}
    public Profile saveProfile(ProfileRequest r){Profile p=profileRepository.findAll().stream().findFirst().orElseGet(Profile::new);p.setName(r.name());p.setHeadline(r.headline());p.setBio(r.bio());p.setEmail(r.email());p.setLocation(r.location());p.setGithubUrl(r.githubUrl());p.setLinkedinUrl(r.linkedinUrl());p.setResumeUrl(r.resumeUrl());p.setImageUrl(r.imageUrl());return profileRepository.save(p);}
    @Transactional(readOnly = true) public List<Project> projects(){return projectRepository.findAllByOrderByDisplayOrderAsc();}

    public Project createProject(ProjectRequest r){Project p=new Project();apply(p,r);p=projectRepository.save(p);featureManagementService.setProjectTechnologies(p.getId(),r.technologyIds().stream().toList());return p;}
    public Project updateProject(Long id,ProjectRequest r){Project p=requireProject(id);apply(p,r);p=projectRepository.save(p);featureManagementService.setProjectTechnologies(p.getId(),r.technologyIds().stream().toList());return p;}
    public void deleteProject(Long id){projectRepository.delete(requireProject(id));}
    private void apply(Project p,ProjectRequest r){p.setTitle(r.title());p.setDescription(r.description());p.setImageUrl(r.imageUrl());p.setGithubUrl(r.githubUrl());p.setLiveUrl(r.liveUrl());p.setFeatured(r.featured());p.setDisplayOrder(r.displayOrder());p.setSkills(new HashSet<>(skillRepository.findAllById(r.skillIds())));}

    @Transactional(readOnly = true) public List<Skill> skills(){return skillRepository.findAllByOrderByNameAsc();}
    ...
    @Transactional(readOnly = true) public List<Experience> experience(){return experienceRepository.findAllByOrderByDisplayOrderAscStartDateDesc();}
    ...
    @Transactional(readOnly = true) public List<Education> education(){return educationRepository.findAllByOrderByDisplayOrderAscStartDateDesc();}
    ...
    @Transactional(readOnly = true) public List<Certification> certifications(){return certificationRepository.findAllByOrderByDisplayOrderAscIssueDateDesc();}
    ...
    @Transactional(readOnly = true) public List<Message> messages(){return messageRepository.findAllByOrderByCreatedAtDesc();}
    ...
    @Transactional(readOnly = true) public long unreadMessages(){return messageRepository.countByReadFalse();}
